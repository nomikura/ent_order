...
func (o *OrganizationQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "priority":
			if _, ok := fieldSeen[organization.FieldPriority]; !ok {
				selectedFields = append(selectedFields, organization.FieldPriority)
				fieldSeen[organization.FieldPriority] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
func (u *UserQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "university":
			if _, ok := fieldSeen[user.FieldUniversity]; !ok {
				selectedFields = append(selectedFields, user.FieldUniversity)
				fieldSeen[user.FieldUniversity] = struct{}{}
			}
		case "organizationID":
			if _, ok := fieldSeen[user.FieldOrganizationID]; !ok {
				selectedFields = append(selectedFields, user.FieldOrganizationID)
